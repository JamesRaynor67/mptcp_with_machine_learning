...
def AnalyzeMonitorSendingRate(monitor_records):
    ...
    c_s_subflow_1, = sns.plt.plot(subflow_rates[0]["Timestamp"].values, subflow_rates[0]["TxRate"].values/1000.0, 'b-', linewidth=2.0) # s->c 1
    # plt.plot(smoothed_time[1], smoothed_tRate[1], 'r-.', linewidth=2.0) # c->s 1
    c_s_subflow_2, = sns.plt.plot(subflow_rates[2]["Timestamp"].values, subflow_rates[2]["TxRate"].values/1000.0, 'r-', linewidth=2.0) # s->c 2
...
def AnalyzeMonitorThroughput(subflow_rates):
    c_s_subflow_1, = sns.plt.plot(subflow_rates[0]["Timestamp"].values, subflow_rates[0]["RxRate"].values/1000.0, 'b-', linewidth=2.0) # s->c 1
    # plt.plot(smoothed_time[1], smoothed_tRate[1], 'r-.', linewidth=2.0) # c->s 1
    c_s_subflow_2, = sns.plt.plot(subflow_rates[2]["Timestamp"].values, subflow_rates[2]["RxRate"].values/1000.0, 'r-', linewidth=2.0) # s->c 2
    # plt.plot(smoothed_time[3], smoothed_tRate[3], 'b-.', linewidth=2.0) # c->s 2
    sns.plt.legend([c_s_subflow_1, c_s_subflow_2], ['Throughput of subflow 1', 'Throughput of subflow 2'], loc='best')
    sns.plt.title('Time-Throughput')
    sns.plt.xlabel('Time / s', fontsize = 14, color = 'black')
    sns.plt.ylabel('Throughput / Kbps', fontsize = 14, color = 'black')
    return subflow_rates

def AnalyzeMonitorSendingRateUtilization(subflow_rates, linkBWList):
    c_s_subflow_1, = sns.plt.plot(subflow_rates[0]["Timestamp"].values, subflow_rates[0]["TxRate"].values/1000.0/linkBWList[0], 'b-', linewidth=2.0) # s->c 1
    c_s_subflow_2, = sns.plt.plot(subflow_rates[2]["Timestamp"].values, subflow_rates[2]["TxRate"].values/1000.0/linkBWList[1], 'r-', linewidth=2.0) # s->c 2
