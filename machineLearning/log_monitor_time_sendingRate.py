...
def AnalyzeMonitorSendingRate(monitor_records):
    ...
    c_s_subflow_1, = sns.plt.plot(smoothed_subflow_rates[0]["Timestamp"].values, smoothed_subflow_rates[0]["TxRate"].values/1000.0, 'b-', linewidth=2.0) # s->c 1
    # plt.plot(smoothed_time[1], smoothed_tRate[1], 'r-.', linewidth=2.0) # c->s 1
    c_s_subflow_2, = sns.plt.plot(smoothed_subflow_rates[2]["Timestamp"].values, smoothed_subflow_rates[2]["TxRate"].values/1000.0, 'r-', linewidth=2.0) # s->c 2
...
def AnalyzeMonitorSendingRateUtilization(subflow_rates, smoothed_subflow_rates, linkBWList):
    ...
    c_s_subflow_1, = sns.plt.plot(smoothed_subflow_rates[0]["Timestamp"].values, smoothed_subflow_rates[0]["TxRate"].values/1000.0/linkBWList[0], 'b-', linewidth=2.0) # s->c 1
    c_s_subflow_2, = sns.plt.plot(smoothed_subflow_rates[2]["Timestamp"].values, smoothed_subflow_rates[2]["TxRate"].values/1000.0/linkBWList[1], 'r-', linewidth=2.0) # s->c 2
