...
#             self._build_net()
#             self.merged_summary_op = tf.summary.merge_all()
#             print "Actor recored in paht: " + self.save_path
#             self.summaryWriter = tf.summary.FileWriter(self.save_path, self.sess.graph)
#         else:
#             self._restore()
...
#         _, exp_v, summary_str = self.sess.run([self.train_op, self.exp_v, self.merged_summary_op], feed_dict)
#         self.summary_writer.add_summary(summary_str, self.iter)
#         self.iter += 1
#         return exp_v
...
#         # td_error, _, summary_str = self.sess.run([self.td_error, self.train_op, self.merged_summary_op],
...
class QLearningTable():
    def __init__(self, actions, learning_rate=0.01, reward_decay=0.9, e_greedy=0.90, save_path=None, restore_path=None):
    ...
    def save(self):
        self.q_table.to_csv(self.save_path)
        print "saved at path: ", self.save_path

    def _restore(self):
        self.q_table = pd.read_csv(self.restore_path, index_col=0)
        # print self.q_table.head(10)
        print "restore from path: ", self.restore_path

class Actor(object):
    def __init__(self, sess, n_features, n_actions, lr=0.01, save_path=None, restore_from_file=None):
        self.sess = sess
        self.restore_from_file = restore_from_file
        self.save_path = save_path
        self.n_features = n_features
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('Actor'):
            with tf.variable_scope('l1'):
                w1 = tf.get_variable('w1', [self.n_features, n_l1], initializer=tf.random_normal_initializer(-0.1, 0.1))
                b1 = tf.get_variable('b1', [1, n_l1], initializer=tf.constant_initializer(0.1))
                l1 = tf.nn.tanh(tf.matmul(self.s, w1) + b1)
            with tf.variable_scope('l2'):
                w2 = tf.get_variable('w2', [n_l1, self.n_actions], initializer=tf.random_normal_initializer(-0.1, 0.1))
                b2 = tf.get_variable('b2', [1, self.n_actions], initializer=tf.constant_initializer(0.1))
                self.acts_prob = tf.nn.softmax(tf.add(tf.matmul(l1, w2), b2), name="acts_prob")
        with tf.variable_scope('exp_v'):
            log_prob = tf.log(self.acts_prob[0, self.a])
...
class Critic(object):
    def __init__(self, sess, n_features, lr=0.01, save_path=None, restore_from_file=None):
        ...
        with tf.variable_scope('Critic'):
            with tf.variable_scope('l1'):
                w1 = tf.get_variable('w1', [n_features, n_l1], initializer=tf.random_normal_initializer(-0.1, 0.1))
                b1 = tf.get_variable('b1', [1, n_l1], initializer=tf.constant_initializer(0.1))
                l1 = tf.nn.relu(tf.matmul(self.s, w1) + b1)

            with tf.variable_scope('l2'):
                w2 = tf.get_variable('w2', [n_l1, 1], initializer=tf.random_normal_initializer(-0.1, 0.1))
                b2 = tf.get_variable('b2', [1, 1], initializer=tf.constant_initializer(0.1))
                self.v = tf.add(tf.matmul(l1, w2), b2, name="V")
...
def extract_observation_for_q_learning(dataRecorder):
    value_dic = dataRecorder.get_latest_data()
    observation = [0]*8
    for i in range(value_dic["nbOfSubflows"]):
        name = "window" + str(i)
        observation[i*4] = int(value_dic[name]/1e3)
        name = "cWnd" + str(i)
        observation[i*4+1] = int(value_dic[name]/1e4)
        name = "rtt" + str(i)
        observation[i*4+2] = int(value_dic[name]/1e5)
        name = "availableTxBuffer" + str(i)
        observation[i*4+3] = int(value_dic[name]/1e4)
    str_observation = ",".join(str(e) for e in observation)
...
#     #             l0 = tf.nn.sigmoid(tf.matmul(self.s, w0) + b0)
...
#     #             w0 = tf.get_variable('w0', [self.n_features, n_l0], initializer=w_initializer, collections=c_names)
#     #             b0 = tf.get_variable('b0', [1, n_l0], initializer=b_initializer, collections=c_names)
#     #             l0 = tf.nn.sigmoid(tf.matmul(self.s_, w0) + b0)
...
#         name = "availableTxBuffer" + str(i)
#         observation[i*4+3] = value_dic[name]
#     observation[8] = value_dic['highTxMarkMeta']- value_dic['lastAckedSeqMeta']
#     observation[9] = value_dic['availableTxBufferMeta']
#     observation[10] = value_dic['unAckMeta']
