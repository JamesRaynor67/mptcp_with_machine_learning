...
def CopyFiles(episode_count):
    ...
    copyfile("/home/hong/workspace/mptcp/ns3/mptcp_output/mptcp_other_server_0", '/home/hong/workspace/mptcp/ns3/rl_training_data/' + str(episode_count) + '_mptcp_other_server_0')
    copyfile("/home/hong/workspace/mptcp/ns3/mptcp_output/mptcp_other_server_1", '/home/hong/workspace/mptcp/ns3/rl_training_data/' + str(episode_count) + '_mptcp_other_server_1')
...
if __name__ == "__main__":
    ...
    parser.add_option("-r", "--rewardByAck", dest="RewardByAck", default=True, help="Reward is calculated by Ack or SentBytes")
    ...
    if options.Algorithm == "DQN":
        ...
        episode_count = 0
        RL = QLearningTable(actions=[0,1,2,3], save_path=os.path.join(options.SavePath, "q-table.csv"))
        ...
        while episode_count < int(options.MaxEpisode):
            ...
            f.write("time,reward\n")
            while True:
                observation_before_action = extract_observation_for_q_learning(dataRecorder)
                ...
                dataRecorder.add_one_record(recv_str)
                observation_after_action = extract_observation_for_q_learning(dataRecorder)
        ...
        episode_count = 0
        sess = tf.Session()
        actor = Actor(sess, n_features=8, n_actions=4, lr=0.01, save_path=options.SavePath, restore_from_file=None)
...
#     if options.Algorithm == "DQN":
#         episode_count = 0
#         RL = DeepQNetwork(n_actions=4, n_features=11, learning_rate=0.01, reward_decay=0.99, e_greedy=0.8, 
#                     replace_target_iter=200, memory_size=1000, output_graph=True, save_path=options.SavePath, restore_from_file=None)
#         while episode_count < int(options.MaxEpisode):
#             rttRecorder = RecordRTT('/home/hong/workspace/mptcp/ns3/rl_training_data/' + str(episode_count) + '_client_rtt')
#             cWndRecorder = RecordCwnd('/home/hong/workspace/mptcp/ns3/rl_training_data/' + str(episode_count) + '_client_cWnd')
#             rWndRecorder = RecordRwnd('/home/hong/workspace/mptcp/ns3/rl_training_data/' + str(episode_count) + '_client_rWnd')
...
#             dataRecorder.add_one_record(recv_str)
#             observation_before_action = extract_observation(dataRecorder)
#             reward = calculate_reward(dataRecorder, reset = True, byAck=True)
...
#                 dataRecorder.add_one_record(recv_str)
#                 observation_after_action = extract_observation(dataRecorder)
#                 reward = calculate_reward(dataRecorder, reset = False, byAck=True)
#                 accumulativeReward += reward
#                 # # Update memory
#                 RL.store_transition(observation_before_action, action, accumulativeReward, observation_after_action)
