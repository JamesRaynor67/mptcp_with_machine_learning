...
if __name__ == "__main__":
    ...
    if options.Algorithm == "DQN":
        ...
        episode_count = 0
        RL = DeepQNetwork(n_actions=4, n_features=11, learning_rate=0.01, reward_decay=0.99, e_greedy=0.9, 
        ...
        while episode_count < int(options.MaxEpisode):
            ...
            action_history, accumulativeReward_history, observation_history = [],[],[]
            ...
            while True:
                ...
                if dataRecorder.get_latest_data()["time"] - lastSchedulerTiming > int(options.SwitchInterval): # in microsecond
                    ...
                    shouldUpdata = True
                    accumulativeReward_history.append(accumulativeReward)
                    accumulativeReward = 0

                if shouldUpdata:
                    observation_history.append(observation_before_action)
                ...
                # # Update memory
                # if shouldUpdata and len(observation_history) > 1:
                if len(observation_history) > 1:
                    RL.store_transition(observation_history[-2], action_history[-2], accumulativeReward_history[-1], observation_history[-1])
                    f.write(str(dataRecorder.get_latest_data()["time"]) + ',' + str(accumulativeReward_history[-1]) + '\n')
