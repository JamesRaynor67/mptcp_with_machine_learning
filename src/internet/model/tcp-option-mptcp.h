...
private:
  //! Unimplemented to avoid misuse
  TcpOptionMpTcpCapable (const TcpOptionMpTcpCapable&) = delete;
  TcpOptionMpTcpCapable& operator= (const TcpOptionMpTcpCapable&) = delete;
};
...
Note that A's token is not required since A launches this connection
...
public:
  ...
  **/
  enum Mode
  {
    Uninitialized = 0, // this state is just for programming in ns3
  ...
  * \return null for now (not implemented) Jiaming: so it returns 0 all the time
  */
  ...
  * \warning Available in Ack mode only
  * \param hmac
  * \todo not implemented Jiaming: in fact implemented?
  */
...
protected:
  ...
  uint32_t m_buffer[5]; /**< Memory buffer to register the data of the different modes, 160 bits */

private:
  //! Defined and unimplemented to avoid misuse
  TcpOptionMpTcpJoin (const TcpOptionMpTcpJoin&) = delete;
  TcpOptionMpTcpJoin& operator= (const TcpOptionMpTcpJoin&) = delete;
};
