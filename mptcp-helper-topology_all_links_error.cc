                        ...
                        NetDeviceContainer& unstableDevices)
{
  ...
  if(useStaticRouting == false){
    ...
    if(g_link_b_BER != 0){                        // Although g_link_b_BER is double, it's still OK to compare it with 0
      std::cout << "Error model installed in link B-C" << std::endl;
      Ptr<RateErrorModel> ptr_em = CreateObjectWithAttributes<RateErrorModel> ();
      ptr_em->SetUnit(RateErrorModel::ERROR_UNIT_BIT);
      ptr_em->SetRate(g_link_b_BER);
      linkedDevices.Get(0)->SetAttribute("ReceiveErrorModel", PointerValue (ptr_em));
      linkedDevices.Get(1)->SetAttribute("ReceiveErrorModel", PointerValue (ptr_em));
    }
    ...
    if(g_link_b_BER != 0){                        // Although g_link_b_BER is double, it's still OK to compare it with 0
      std::cout << "Error model installed in link C-D" << std::endl;
      Ptr<RateErrorModel> ptr_em = CreateObjectWithAttributes<RateErrorModel> ();
      ptr_em->SetUnit(RateErrorModel::ERROR_UNIT_BIT);
      ptr_em->SetRate(g_link_b_BER);
      linkedDevices.Get(0)->SetAttribute("ReceiveErrorModel", PointerValue (ptr_em));
    ...
    if(g_link_b_BER != 0){                        // Although g_link_b_BER is double, it's still OK to compare it with 0
      std::cout << "Error model installed in link C-F" << std::endl;
      Ptr<RateErrorModel> ptr_em = CreateObjectWithAttributes<RateErrorModel> ();
      ptr_em->SetUnit(RateErrorModel::ERROR_UNIT_BIT);
      ptr_em->SetRate(g_link_b_BER);
      linkedDevices.Get(0)->SetAttribute("ReceiveErrorModel", PointerValue (ptr_em));
    ...
    if(g_link_b_BER != 0){                        // Although g_link_b_BER is double, it's still OK to compare it with 0
      std::cout << "Error model installed in link D-H" << std::endl;
      Ptr<RateErrorModel> ptr_em = CreateObjectWithAttributes<RateErrorModel> ();
      ptr_em->SetUnit(RateErrorModel::ERROR_UNIT_BIT);
      ptr_em->SetRate(g_link_b_BER);
      linkedDevices.Get(0)->SetAttribute("ReceiveErrorModel", PointerValue (ptr_em));
    ...
    // linkedDevices = PointToPointCreate(D, E, DataRate("10Mbps"), Time(g_link_b_delay), packetSize, g_router_b_buffer_size);
    Ipv4InterfaceContainer deInterfaces = addressHelper.Assign(linkedDevices);
    if(g_link_b_BER != 0){                        // Although g_link_b_BER is double, it's still OK to compare it with 0
      std::cout << "Error model installed in link D-E" << std::endl;
      Ptr<RateErrorModel> ptr_em = CreateObjectWithAttributes<RateErrorModel> ();
      ptr_em->SetUnit(RateErrorModel::ERROR_UNIT_BIT);
      ptr_em->SetRate(g_link_b_BER);
      linkedDevices.Get(0)->SetAttribute("ReceiveErrorModel", PointerValue (ptr_em));
    ...
    if(g_link_b_BER != 0){                        // Although g_link_b_BER is double, it's still OK to compare it with 0
      std::cout << "Error model installed in link E-J" << std::endl;
      Ptr<RateErrorModel> ptr_em = CreateObjectWithAttributes<RateErrorModel> ();
      ptr_em->SetUnit(RateErrorModel::ERROR_UNIT_BIT);
      ptr_em->SetRate(g_link_b_BER);
      linkedDevices.Get(0)->SetAttribute("ReceiveErrorModel", PointerValue (ptr_em));
    ...
    if(g_link_b_BER != 0){                        // Although g_link_b_BER is double, it's still OK to compare it with 0
      std::cout << "Error model installed in link F-I" << std::endl;
      Ptr<RateErrorModel> ptr_em = CreateObjectWithAttributes<RateErrorModel> ();
      ptr_em->SetUnit(RateErrorModel::ERROR_UNIT_BIT);
      ptr_em->SetRate(g_link_b_BER);
      linkedDevices.Get(0)->SetAttribute("ReceiveErrorModel", PointerValue (ptr_em));
    ...
    // linkedDevices = PointToPointCreate(F, G, DataRate("10Mbps"), Time(g_link_c_delay), packetSize, g_router_c_buffer_size);
    Ipv4InterfaceContainer fgInterfaces = addressHelper.Assign(linkedDevices);
    if(g_link_b_BER != 0){                        // Although g_link_b_BER is double, it's still OK to compare it with 0
      std::cout << "Error model installed in link F-G" << std::endl;
      Ptr<RateErrorModel> ptr_em = CreateObjectWithAttributes<RateErrorModel> ();
      ptr_em->SetUnit(RateErrorModel::ERROR_UNIT_BIT);
      ptr_em->SetRate(g_link_b_BER);
      linkedDevices.Get(0)->SetAttribute("ReceiveErrorModel", PointerValue (ptr_em));
    ...
    if(g_link_b_BER != 0){                        // Although g_link_b_BER is double, it's still OK to compare it with 0
      std::cout << "Error model installed in link G-K" << std::endl;
      Ptr<RateErrorModel> ptr_em = CreateObjectWithAttributes<RateErrorModel> ();
      ptr_em->SetUnit(RateErrorModel::ERROR_UNIT_BIT);
      ptr_em->SetRate(g_link_b_BER);
      linkedDevices.Get(0)->SetAttribute("ReceiveErrorModel", PointerValue (ptr_em));
    ...
    if(g_link_b_BER != 0){                        // Although g_link_b_BER is double, it's still OK to compare it with 0
      std::cout << "Error model installed in link G-A" << std::endl;
      Ptr<RateErrorModel> ptr_em = CreateObjectWithAttributes<RateErrorModel> ();
      ptr_em->SetUnit(RateErrorModel::ERROR_UNIT_BIT);
      ptr_em->SetRate(g_link_b_BER);
      linkedDevices.Get(0)->SetAttribute("ReceiveErrorModel", PointerValue (ptr_em));
